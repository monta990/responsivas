...
use GlpiPlugin\Responsivas\Generator;
...
final class MailService
{
    public static function handle(): Response
    {
        ...
        if (!\Session::haveRight('user', READ)) {
            ...
            return new RedirectResponse(rtrim($CFG_GLPI['root_doc'] ?? '', '/') . '/front/user.php');
        }
...
if (($_POST['mode'] ?? '') === 'test') {
   ...
   if (!\Session::haveRight('config', UPDATE)) {
      \Session::addMessageAfterRedirect(__('Access denied.', 'responsivas'), false, ERROR);
      return new \Symfony\Component\HttpFoundation\RedirectResponse(Paths::routeUrl('config'));
   }

   $redirect_url = Paths::routeUrl('config');
   ...
   $email_subject = '[PRUEBA] ' . strip_tags(Utils::applyTemplate($email_subject_tpl, $vars));
   $body_safe     = nl2br(Utils::applyTemplate($email_body_tpl, $vars));
   $footer_raw    = Utils::applyTemplate($email_footer_tpl, $vars);
...
HTML;
   ...
   try {
      ...
      \Toolbox::logInFile('responsivas', '[test-mail] ' . $e->getMessage() . "\n");
      \Session::addMessageAfterRedirect(
...
$redirect_user = static function (int $uid) use ($CFG_GLPI): \Symfony\Component\HttpFoundation\Response {
   return new \Symfony\Component\HttpFoundation\RedirectResponse(
      rtrim($CFG_GLPI['root_doc'] ?? '', '/') . '/front/user.form.php?id=' . $uid
   );
};

if ($user_id <= 0) {
   \Session::addMessageAfterRedirect(__('Invalid user.', 'responsivas'), false, ERROR);
   return new \Symfony\Component\HttpFoundation\RedirectResponse($CFG_GLPI['root_doc'] . '/front/user.php');
}
...
// responsivasApplyTemplate: ** * __ en orden correcto, luego sustituye variables
$email_subject = strip_tags(Utils::applyTemplate($email_subject_tpl, $vars));
...
$pdfs = Generator::generateSelected($user_id, [
...
try {
   ...
   \Toolbox::logInFile('responsivas', '[mail] ' . $e->getMessage() . "\n");
   \Session::addMessageAfterRedirect(
      __('Error sending email. Check the GLPI error log for details.', 'responsivas'),
      false,
      ERROR
