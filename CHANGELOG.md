...
- **Configuration backup** — Added administrator-only JSON export/import for Responsivas configuration, including templates, document/email/footer settings and the current logo. Imports validate the file format, size, supported keys, numeric bounds, timezone and referenced GLPI objects before applying the configuration.
- **`{precio}` variable for computer responsibilities** — Computer responsibility templates now expose `{precio}` using the purchase price stored in GLPI's administrative and financial information. It uses the configured currency and the same two-decimal formatting used by phone responsibilities. The variable is available in the computer title, introduction, body, and useful-life paragraph templates, and is shown in the computer variable hints.
...
- **Modern GLPI architecture** — Migrated runtime classes to PSR-4 under `src/`, replaced legacy `front/` runtime endpoints with GLPI/Symfony Controllers, centralized configuration, mail and update logic into services, and removed the legacy `inc/` and `front/` runtime directories while preserving existing configuration keys, PDF output, templates, permissions, and GLPI 11/12 compatibility.
- **GLPI 12 compatibility fixes** — Replaced removed `Toolbox::logError()` usage with `Toolbox::logInFile()`, corrected namespaced `RuntimeException` catches and `CommonDBTM`/`User` type hints in the PDF builder, and ensured preview watermark state is reset after real-asset generation errors.
- **User tab compatibility** — Restored Controller route URLs passed to the user tab for computer, printer, phone and email actions, and corrected `UserTab` method type hints to reference GLPI core `\CommonGLPI` and `\User` classes.
- **Entity location validation** — Responsibility previews and real PDF generation now stop with a translated warning when the active GLPI entity is missing City or State, instead of silently using incomplete or fallback location data. Country remains optional and is included when configured.
- **Preview location consistency** — Laptop and printer previews now use the active GLPI entity location (city, state and country), matching the phone preview and real document generation; hardcoded preview location fallback was removed.
- **Computer responsibility PDF layout** — Reorganized the computer asset table for clearer reading: core equipment data is presented first, the comments section spans the full table width, and associated monitors/peripherals are grouped under a dedicated **Associated devices** section with their own column headers.
- **Computer useful-life paragraphs** — Added two configurable optional paragraphs for computers: **Useful life paragraph (with invoice)** and **Useful life paragraph (without invoice)**, using the same invoice/supplier detection model as phones. The paragraphs support `{fecha_compra}`, `{factura}`, `{proveedor}` and `{clausula_vida_util}`.
- **Optional useful-life behavior** — Useful-life text is inserted only when the corresponding configured field contains text. If the applicable field is empty, no paragraph is generated. The paragraph is placed in the existing computer responsibility body rather than being created as a new numbered clause.
- **Existing template compatibility** — Existing computer templates that do not yet contain `{clausula_vida_util}` receive the optional paragraph at the intended position in the body, immediately before the vacation-period paragraph, without introducing a new clause number.
- **Computer preview/demo** — The computer preview now demonstrates the useful-life paragraph using demo invoice, supplier and purchase-date data. If a custom invoice paragraph is configured, the preview uses that text; otherwise it displays the built-in demo example.
- **Purchase date formatting** — `{fecha_compra}` now uses the same localized long-date format as `{fecha}` in computer and phone responsibilities, for example `18 de agosto de 2026`, instead of a numeric date format.
- **Translations** — Updated and synchronized the plugin translation catalogs (`.pot`, `.po` and compiled `.mo` files) for the new computer useful-life paragraph options, purchase-date formatting, and current runtime messages. Obsolete catalog entries were removed and the POT/PO project header was normalized to `Responsivas`.
